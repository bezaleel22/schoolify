<div class="modal fade admin-query" id="perf_madal">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">@lang('result::student.performance_title')</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="container-fluid">
                    <form action="{{ route('result.rating') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12">
                                <input type="hidden" name="student_id" value="{{ $student_detail->id }}">
                                <input type="hidden" id="examTypeId" name="exam_type_id">
                                <div class="row mt-10">
                                    <div class="col-lg-4">
                                        <label>@lang('result::student.opened') <span>*</span> </label>
                                        <input value="{{ $results->student->opened }}" class="primary_input_field form-control" type="number">
                                    </div>
                                    <div class="col-lg-4">
                                        <label>@lang('result::student.present') <span>*</span> </label>
                                        <input value="{{ $results->student->present }}" class="primary_input_field form-control" type="number">
                                    </div>
                                    <div class="col-lg-4">
                                        <label>@lang('result::student.absent') <span>*</span> </label>
                                        <input value="{{ $results->student->absent }}" class="primary_input_field form-control" type="number">
                                    </div>
                                </div>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="input-effect">
                                            ...
                                            <select class="primary_input_field form-control" id="attributeSelect">
                                                ...
                                                @foreach($results->ratings as $rating)
                                                <option value="{{ $rating->attribute }}" @if(old('ratings') && array_key_exists($rating->attribute, old('ratings'))) disabled @endif>
                                                    {{ $rating->attribute }}
                                                </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="col-lg-12 mt-3">
                                    <button type="button" class="primary-btn fix-gr-bg" id="addRatingButton">@lang('common.add')</button>
                                </div>

                                <!-- Display Added Ratings Inline -->
                                <div class="col-lg-12 mt-4">
                                    <h5>@lang('result::student.added_ratings')</h5>
                                    <div id="addedRatingsContainer" class="d-flex flex-wrap align-items-center">
                                        <!-- Prepopulate existing ratings from database -->
                                        @foreach($results->ratings as $key => $rating)
                                        <div class="tag d-inline-flex align-items-center m-2 p-2 bg-light border rounded">
                                            ...
                                            <button class="btn btn-sm text-danger remove-rating" data-attribute="{{ $rating->attribute }}">&times;</button>

                                            <!-- Hidden inputs to submit -->
                                            <input type="hidden" name="ratings[{{ $key }}][attribute]" value="{{ $rating->attribute }}">
                                            <input type="hidden" name="ratings[{{ $key }}][rate]" value="{{ $rating->rate }}">
                                        </div>
                                        @endforeach
                                    </div>
                                </div>
...
<script>
    // Function to disable prepopulated ratings in the dropdown
    function disablePrepopulatedRatings() {
        const existingRatings = @json(collect($results - > ratings) - > pluck('attribute') - > toArray());
        const attributeSelect = document.getElementById('attributeSelect');

        // Loop through options and disable if they exist in the existing ratings
        for (const option of attributeSelect.options) {
            if (existingRatings.includes(option.value)) {
    ...
    $('#perf_madal').on('show.bs.modal', function() {
        disablePrepopulatedRatings();
    });
    ...
    document.getElementById('addRatingButton').addEventListener('click', function() {
        ...
        if (attribute && rating) {
            ...
            tag.innerHTML = `
                ...
                <button class="btn btn-sm text-danger remove-rating" data-attribute="${attribute}">&times;</button>
