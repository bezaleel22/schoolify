...
<head>
  ...
  <body class="w-full h-full bg-custom">
    <header class="lg:flex lg:justify-between mb-8 w-full font-normal print:flex print:justify-between print:space-x-3">
        @include('template.header', ['school' => $school])
    </header>
    <div class="flex flex-row">
        @include('template.student_info', ['student' => $student])
    </div>
    @include('template.record', ['records' => $records])
    @include('template.score', ['score' => $score])

    @if ($student->type == 'GRADERS')
    @include('template.rating', ['ratings' => $ratings])
    @endif

    @include('template.remark', ['remark' => $remark])
</body>
